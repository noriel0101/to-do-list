  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:20px_20px] flex flex-col items-center p-6 md:p-12 text-slate-800">
      ...
      <div className="w-full max-w-2xl flex justify-between items-end mb-10">
        <div>
          <h1 className="text-4xl font-black text-slate-900">Focus Hub</h1>
          <p className="text-slate-500">Your workspace.</p>
        </div>
      ...
      <div className="w-full max-w-2xl bg-white/70 backdrop-blur-md p-6 rounded-[2rem] shadow-sm border border-slate-200/50 mb-8">
        <form onSubmit={add} className="flex gap-3">
          <input className="flex-1 bg-white border border-slate-200 p-4 rounded-xl outline-none text-lg" placeholder="New board title..." value={newList} onChange={e => setNewList(e.target.value)} />
        ...
        {lists.map(l => (
          <div key={l.id} className="flex justify-between items-center p-5 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-all">
            <div className="flex-1">
              {editingId === l.id ? (
                <input autoFocus className="text-lg font-bold text-slate-800 outline-none w-full bg-slate-50 px-2 rounded-lg" value={editingTitle} onChange={e => setEditingTitle(e.target.value)} onBlur={() => save(l.id)} onKeyDown={e => e.key === 'Enter' && save(l.id)} />
              ) : (
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl max-w-sm w-full text-center">
            ...
            <p className="text-slate-500 mb-8">Delete this board permanently?</p>
